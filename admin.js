...
function renderList() {
  $('#project-list').innerHTML = state.content.projects.map((project) => {
    const media = project.mediaType === 'video' ? `<video src="${escapeHtml(project.media)}" muted preload="none"></video>` : project.media ? `<img src="${escapeHtml(project.media)}" alt="" loading="lazy" decoding="async" />` : '<span class="media-placeholder"></span>';
